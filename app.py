...
with top_col1:
    role_label = (
        "👑 管理員 (無限次數)" if st.session_state.user_role == "admin" else "👤 使用者"
    )
...
def build_system_prompt(mode="full"):
    ...
    - 著重提供關鍵觀念、解題切演點、公式推導方向或邏輯陷阱，引導使用者自行推理。
    """
        if mode == "hint"
        else """
    【特別指令 - 完整解析模式】：
...
if menu_option == "⚙️ 系統管理":
    ...
    for u_name, info in list(st.session_state.users_db.items()):
        ...
        with st.expander(
            ...
            with col_a:
                st.write(
                    f"**密碼狀態**：{'預設密碼(需變更)' if info.get('first_login') else '已自訂密碼'}"
                )
            ...
            with col_b:
                ...
                if st.button("💾 儲存該生獨立額度", key=f"save_limit_{u_name}"):
                    st.session_state.users_db[u_name][
                        "custom_limit"
                    ] = custom_limit_val
                    save_config_from_session()
            ...
            if btn_c4.button("🗑️ 刪除帳號", key=f"del_{u_name}"):
...
# 📝 開始解題頁面 (核心功能)
    ...
    if "latest_result" in st.session_state and st.session_state.latest_result:
        ...
        st.divider()
        st.markdown(
            '<div class="step-header"><span class="step-number">3</span>觀念解析與交叉驗證</div>',
            unsafe_allow_html=True,
        )
