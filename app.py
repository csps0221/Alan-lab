...
# 初始化學生帳號資料庫 (結構: {"姓名": {"password": "密碼", "first_login": True/False}})
if "students_db" not in st.session_state:
    st.session_state.students_db = {
        "王小明": {"password": "123", "first_login": True},
        "張小華": {"password": "123", "first_login": True},
        "測試學生": {"password": "111", "first_login": True}
    }
...
if st.session_state.must_change_password:
    ...
    if st.button("確認更新密碼", type="primary"):
        ...
        elif pwd1 == "123" or pwd1 == "111":
            st.warning("新密碼不能與預設密碼相同，請設定更有保障的密碼！")
        else:
...
if st.session_state.user_role == "admin":
    with st.expander("⚙️ 管理員選單：學生帳號與密碼重置管理", expanded=True):
        st.subheader("➕ 新增學生帳號")
        c1, c2 = st.columns(2)
        new_name = c1.text_input("學生姓名", key="new_s_name")
        new_pass = c2.text_input("預設密碼", value="123", key="new_s_pass")
        ...
        if st.button("新增學生帳號"):
            if new_name and new_pass:
                ...
                st.success(f"已成功新增學生：{new_name}（預設密碼：{new_pass}，登入時會強制要求更改）")
                st.rerun()
            else:
                st.warning("請填寫姓名與預設密碼！")
        
        st.divider()
        st.subheader("📋 目前學生名單與權限控制")
        
        for s_name, info in list(st.session_state.students_db.items()):
            col_a, col_b, col_c, col_d = st.columns([2, 2, 2, 1])
            col_a.write(f"**姓名**：{s_name}")
            status_str = "⚠️ 未設定新密碼" if info["first_login"] else "✅ 已完成設定"
            col_b.write(f"**狀態**：{status_str}")
            
            # 還原/重置密碼按鈕
            if col_c.button("🔄 還原密碼", key=f"reset_{s_name}"):
                st.session_state.students_db[s_name]["password"] = "123"
                st.session_state.students_db[s_name]["first_login"] = True
                st.toast(f"已將 {s_name} 的密碼重置為 123！")
                st.rerun()
                
            # 刪除學生按鈕
            if col_d.button("🗑️ 刪除", key=f"del_{s_name}"):
